      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.vehicleFormCard}>
          ...
          <View style={styles.toggleRow}>
            <TouchableOpacity
              style={[styles.toggleButton, !isVisitor && styles.toggleButtonActive]}
              onPress={() => setIsVisitor(false)}
            >
              <Text style={[styles.toggleButtonText, !isVisitor && styles.toggleButtonTextActive]}>일반 차량</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.toggleButton, isVisitor && styles.toggleButtonVisitorActive]}
              onPress={() => setIsVisitor(true)}
            >
              <Text style={[styles.toggleButtonText, isVisitor && styles.toggleButtonTextActive]}>방문 차량</Text>
            </TouchableOpacity>
          </View>
          {isVisitor && (
            <TextInput
        ...
        <Text style={styles.sectionTitle}>등록된 차량</Text>
        {vehicleLoading ? (
          ...
          vehicles.map((v) => (
            <View key={v.id} style={styles.vehicleCard}>
              <View style={styles.vehicleCardLeft}>
                ...
                {v.modelName ? <Text style={styles.vehicleModel}>{v.modelName}</Text> : null}
                <View style={styles.vehicleBadgeRow}>
                  <View style={[styles.badge, v.isVisitor ? styles.badgeVisitor : styles.badgeRegular]}>
                    <Text style={styles.badgeText}>{v.isVisitor ? '방문차량' : '일반차량'}</Text>
                  </View>
                </View>
              ...
              <TouchableOpacity style={styles.deleteButton} onPress={() => handleDeleteVehicle(v.id)}>
...
const styles = StyleSheet.create({
  ...
  input: {
    height: 48, borderWidth: 1, borderColor: '#E5E5EA', borderRadius: 10,
    paddingHorizontal: 14, fontSize: 15, color: '#1C1C1E', backgroundColor: '#F8F9FA', marginBottom: 12,
  },
  ...
  toggleButton: {
    ...
    borderWidth: 1, borderColor: '#E5E5EA', backgroundColor: '#F8F9FA',
  },
  toggleButtonActive: { backgroundColor: '#007AFF', borderColor: '#007AFF' },
  toggleButtonVisitorActive: { backgroundColor: '#FF9500', borderColor: '#FF9500' },
  toggleButtonText: { fontSize: 14, fontWeight: '600', color: '#8E8E93' },
  toggleButtonTextActive: { color: '#fff' },
  registerButton: {
    backgroundColor: '#007AFF', height: 50, borderRadius: 12,
    justifyContent: 'center', alignItems: 'center', marginTop: 4,
  },
  registerButtonDisabled: { backgroundColor: '#A8C7FA' },
  registerButtonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  emptyCard: {
    backgroundColor: '#fff', borderRadius: 16, padding: 20, alignItems: 'center', marginBottom: 16,
  ...
  vehicleCardLeft: { flex: 1, marginRight: 12 },
  vehiclePlate: { fontSize: 18, fontWeight: '700', color: '#1C1C1E', marginBottom: 6 },
  vehicleModel: { fontSize: 13, color: '#8E8E93', marginBottom: 4 },
  vehicleBadgeRow: { flexDirection: 'row', marginBottom: 4 },
  badge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  badgeRegular: { backgroundColor: '#E3F2FD' },
  badgeVisitor: { backgroundColor: '#FFF3E0' },
  badgeText: { fontSize: 11, fontWeight: '700', color: '#3A3A3C' },
  vehicleDeparture: { fontSize: 12, color: '#FF9500', marginTop: 2 },
  deleteButton: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: '#FF3B30' },
