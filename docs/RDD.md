...
| F-46 | 댓글 작성·조회 | 2 | ⬜ | |
| F-47 | 내가 쓴 글 조회 | 2 | ⬜ | |
| F-48 | 게시글 이미지 첨부 | 2 | ⬜ | `<input type="file">` + Supabase Storage |
| F-49 | 댓글 푸시 알림 | 3 | ⬜ | |
| F-50 | 게시글 좋아요 | 3 | ⬜ | |
...
| # | 요구사항 | Phase | 상태 | 비고 |
|---|---------|-------|------|------|
| F-54 | 투표 생성 (제목/선택지/종료일/익명 여부) | 2 | ⬜ | |
| F-55 | 투표 참여 (라디오 선택) | 2 | ⬜ | HEAD 세대주 전용 |
| F-56 | 1세대 1표 강제 (`@@unique([pollId, roomNumber])`) | 2 | ⬜ | DB + 서버 이중 검증 |
| F-57 | 투표 결과 시각화 (퍼센트 바, 기명 시 호수 표시) | 2 | ⬜ | |
| F-58 | 투표 참여율 프로그레스 바 | 2 | ⬜ | |
| F-59 | 투표 수정 (마감 전) | 2 | ⬜ | |
| F-60 | 미참여자 독촉 알림 | 2 | ⬜ | |
...
| # | 요구사항 | Phase | 상태 | 비고 |
|---|---------|-------|------|------|
| F-62 | 공용 지출 장부 조회 (입주민 투명성) | 2 | ⬜ | `LedgerTransaction` DB 직접 연동 |
| F-63 | 수입·지출 내역 등록 (동대표) | 2 | ⬜ | |
| F-64 | 영수증 첨부 (Supabase Storage) | 2 | ⬜ | |
...
| # | 요구사항 | Phase | 상태 | 비고 |
|---|---------|-------|------|------|
| F-70 | 차량 등록 (일반/방문, 모델명, 출차 예정) | 2 | ⬜ | |
| F-71 | 번호판 검색 → 호수·이름·방문여부 표시 | 2 | ⬜ | |
