...
| Sprint 16 | 2026-05-05 | F-94 차량 이동 요청 푸시 (이중주차 넛지), F-95 전출 정산 일할 계산기, F-96 순환형 공동 당번 + 정기 점검 스케줄러, F-99 다중 빌라 퀵스위치 드롭다운 |
...
| F-95 | 전출 정산 일할 계산기 | 입주민 관리 > 정산 버튼 → 이사 날짜 입력 → 당월 관리비 일할 계산(올림) → ExternalBilling 생성 → 결제 링크 복사 공유 | ✅ | ExternalBilling 모델 재활용. 일할 계산: Math.ceil(fixedFee × usedDays / totalDays) |
| F-96 | 순환형 공동 당번 + 정기 점검 스케줄러 | ① 세대 순환 당번: 참여 세대 목록·시작일·주기(주간/격주) 설정 → 당번 교체일에 자동 푸시 알림. ② 정기 점검 리마인더: 소화기·승강기·가스 등 법정 점검 주기 설정 → D-30/D-7 자동 알림 (관리자에게 발송) | ✅ | `DutySchedule`, `DutyRule` DB 모델 신규 추가. Cron 인프라 기존 활용 |
