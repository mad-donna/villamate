    ...
    <main className="min-h-screen bg-neutral-50 pb-24">
      ...
      <div className="flex items-center justify-center gap-4 px-4 mb-4">
        <button
          ...
          aria-label="이전 달"
        >
          <svg className="w-5 h-5 text-neutral-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="px-4 mb-4">
          <div className="bg-white rounded-2xl shadow-sm p-4 space-y-4">
            ...
            <div>
              ...
              {receiptPreview ? (
                <div className="relative w-full">
                  ...
                  <img src={receiptPreview} alt="영수증 미리보기" className="w-full max-h-48 object-cover rounded-xl border border-neutral-200" />
                  {uploading && (
                    <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-white/70">
                      <svg className="animate-spin w-6 h-6 text-primary-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  ...
                  {!uploading && (
                    <button onClick={handleRemoveReceipt} className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center rounded-full bg-neutral-800/70 text-white hover:bg-neutral-900 transition-colors" aria-label="영수증 제거">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="w-full min-h-[80px] flex flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-neutral-300 hover:border-primary-400 hover:bg-primary-50 transition-colors text-neutral-400 hover:text-primary-600">
              ...
              {receiptUrl && !uploading && <p className="text-xs text-green-600 mt-1 font-medium">업로드 완료</p>}
            </div>
            ...
            <div className="flex gap-2 pt-1">
              <Button variant="secondary" size="md" className="flex-1" onClick={handleCancelForm} disabled={submitting}>
                취소
              </Button>
              <Button variant="primary" size="md" className="flex-1" loading={submitting} disabled={uploading} onClick={handleSubmit}>
                {uploading ? '업로드 중...' : '등록'}
              </Button>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div className="px-4 mb-4 grid grid-cols-3 gap-2">
          <div className="bg-white rounded-2xl shadow-sm p-3">
            ...
            <p className="text-sm font-bold text-green-600 leading-tight">{formatAmount(summary.totalIncome)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm p-3">
            <p className="text-xs font-medium text-neutral-500 mb-1">지출</p>
            <p className="text-sm font-bold text-red-500 leading-tight">{formatAmount(summary.totalExpense)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm p-3">
            <p className="text-xs font-medium text-neutral-500 mb-1">잔액</p>
            <p className="text-sm font-bold text-primary-600 leading-tight">{formatAmount(summary.balance)}</p>
          </div>
        </div>
      ) : null}
      ...
      <div className="px-4 space-y-2">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <svg className="w-12 h-12 text-neutral-300 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {tx.receiptUrl && (
                      <button onClick={() => setViewerSrc(tx.receiptUrl!)} className="text-xs text-primary-600 hover:underline text-left">
                        영수증 보기
                      </button>
                    )}
                  </div>
                ...
                <div className="flex flex-col items-end shrink-0 gap-1">
                  ...
                  <span className={tx.type === 'INCOME' ? 'text-sm font-bold text-green-600' : 'text-sm font-bold text-red-500'}>
      ...
      {/* 토스트 */}
      {toast && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-fit px-5 py-3 bg-neutral-800 text-white text-sm font-medium rounded-2xl shadow-lg z-50">
          {toast}
        </div>
      )}
    </main>
