    ...
    <main className="min-h-screen bg-neutral-50 pb-24">
      <div className="flex items-center justify-between px-4 pt-6 pb-4">
        <div>
          <button
            type="button"
            onClick={() => router.back()}
            className="flex items-center gap-1 text-sm text-neutral-500 mb-1 hover:text-neutral-700"
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          ...
          <Button variant="ghost" size="sm" className="mt-3" onClick={fetchPolls}>다시 시도</Button>
        </div>
      ) : polls.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          ...
          <p className="text-sm text-neutral-400 mt-1">첫 번째 투표를 생성해보세요.</p>
        </div>
      ) : (
          ...
          {active.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {active.map((poll) => (
                  <PollCard key={poll.id} poll={poll} onClick={() => router.push(`/manage/polls/${poll.id}`)} />
                ))}
              </div>
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 text-xs text-neutral-400 mt-2">
        ...
        <span>·</span>
        <span>참여 {poll.voteCount}세대</span>
        <span>·</span>
